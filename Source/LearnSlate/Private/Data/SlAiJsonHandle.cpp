...
SlAiJsonHandle::SlAiJsonHandle()
{
	...
	CompoundTableName = FString("CompoundTable.json");
...
void SlAiJsonHandle::ResourceAttrJsonRead(TMap<int, TSharedPtr<ResourceAttribute>>& ResourceAttrMap)
{
	...
	if (FJsonSerializer::Deserialize(JsonReader, JsonParsed))
	{
		for (int i = 0 ; i < JsonParsed.Num(); ++i)
		{
			...
			{
				FString FlobObjectItem = FlobObjectInfo[j]->AsObject()->GetStringField(FString::FromInt(j));
				FString ObjectIndexStr;
				FString RangeStr;
				FString RangeMinStr;
				FString RangeMaxStr;
				FlobObjectItem.Split(FString("_"), &ObjectIndexStr, &RangeStr);
...
void SlAiJsonHandle::CompundTableJsonRead(TArray<TSharedPtr<CompoundTable>>& CompoundTableMap)
{
	FString JsonValue;
	LoadStringFromFile(CompoundTableName, RelativePath, JsonValue);
